"""The declared floor, the README, and the guard's own constant say one thing.
...
* the README's `Python 3.6+`, which was a badge until the static badges were dropped.
  Prose nobody greps is the copy that keeps saying 3.6 after a floor change.
...
def readme_floor() -> str:
    """The floor the README states, as in `Python 3.6+`."""
...
class TestTheFloorIsOneNumberEverywhere:
    def test_the_readme_matches_the_declared_floor(self) -> None:
        assert readme_floor() == declared_floor(), (
            f"README says Python {readme_floor()}+, pyproject requires >={declared_floor()}; "
            "the README is the surface nobody greps"
        )
...
class TestControls:
    ...
    def test_every_surface_was_actually_found(self) -> None:
        for value in (readme_floor(), declared_floor(), guard_floor()):
            assert re.fullmatch(r"\d+\.\d+", value), value

    def test_the_readme_still_states_its_other_claims(self) -> None:
        text = README.read_text()
        assert "No dependencies" in text and "MIT" in text
